...
const hypotheses = [
  { label: "Bad deploy", state: "refuted", note: "git history quiet for 14h" },
  { label: "DB pool exhausted", state: "confirmed", note: "pg_stat_activity diff +812" },
  { label: "Upstream provider", state: "refuted", note: "downstream peers green" },
        ...
        >
          ...
          <motion.h2 variants={fadeUp} className="heading-lg mt-4">
            ...
            <span className="text-ink-300">
              Correlate every signal simultaneously. Land the answer in
              seconds, not hours.
            </span>
          </motion.h2>
          <motion.p variants={fadeUp} className="lead mt-5">
            Manual investigation is sequential — open a dashboard, eyeball
            a chart, page a colleague, repeat. HealOps fans out the
            investigation across every candidate failure mode at once, lets
            the evidence vote, and stops the moment one hypothesis is
        ...
        <div className="mt-14 grid lg:grid-cols-[1fr_1fr] gap-6">
          ...
          >
              ...
              {hypotheses.map((h, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <GitBranch
                      className={`size-4 ${
                        h.state === "confirmed" ? "text-heal-400" : "text-ink-500"
                      }`}
                    />
                    <span
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={stateChip[h.state]}>{h.state}</span>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-5 flex items-center gap-2 text-ink-300 text-sm">
              ...
              Decisively confirmed in 8s — three branches discarded with
              evidence, one survived with proof.
            </div>
          </motion.div>
          ...
          >
            ...
            <p className="mt-3 text-ink-200 leading-relaxed">
              Logs in Datadog, metrics in Grafana, configs in Git, runbooks
              in Notion, post-mortems in Slack. Each tool saw part of what
              happened. None of them saw the whole thing. HealOps reads
              them all and stitches the evidence into one timeline — the
              same one that ships in the PR.
            </p>

            <div className="mt-6 grid grid-cols-2 gap-2">
              {[
                { label: "Observability", v: "Datadog · Grafana · Sentry" },
                { label: "Infrastructure", v: "AWS · k8s · Vercel" },
                { label: "Source", v: "GitHub · GitLab" },
                { label: "Team knowledge", v: "Slack · Notion · runbooks" },
              ].map((row) => (
                <div
                  key={row.label}
                ...
                >
                  ...
                  <div className="mt-1 text-[12.5px] text-ink-200">{row.v}</div>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-ink-200 leading-relaxed max-w-2xl">
                The verified fix becomes a runbook entry, a regression
                test, and a memorised pattern. Re-occurrences are
                investigated in a fraction of the original time — or
                prevented before the page ever fires. HealOps gets quieter
                every quarter, on purpose.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-3 sm:flex sm:flex-col gap-3 lg:min-w-[200px]">
            <Stat label="Repeat incidents" value="↓ 73%" tone="heal" />
            <Stat label="P1 pages / wk" value="↓ 41%" tone="heal" />
            <Stat label="MTTR drift" value="−0.4× / qtr" tone="signal" />
          </div>
        </motion.div>
      </div>
    </section>
